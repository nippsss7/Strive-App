...
import { motion } from "framer-motion"; // NOT motion/react for JS
import { cn } from "@/lib/utils"; // You can replace this with `clsx` or remove it
...
export function LampContainer({ children, className }) {
  return (
    <div className={` lamp-height-fix pt-[12rem] relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-slate-950 w-full rounded-md z-0 ${className || ""}`}>
      ...
      <div className="relative z-50 flex -translate-y-72 sm:-translate-y-96 flex-col items-center px-5">
        {children}
      </div>
    </div>
  );
